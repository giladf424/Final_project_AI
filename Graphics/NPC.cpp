...
Cell* NPC::CheckNeighbor(int r, int c, Cell* pCurrent, Position target, priority_queue<Cell*, vector<Cell*>, CompareCells>& grays, int dupMaze[MSZ][MSZ], double dupMap[MSZ][MSZ], int* numSteps)
{
	if (r == target.row && c == target.col)
	{
		...
		{
			std::cout << "The target in the next position (" << r << ", " << c << ")" << endl;
			*numSteps = 1;
...
Cell* NPC::RestorePath(Cell* pc, int* numSteps)
{
	*numSteps = 2;
	...
	std::cout << "Radius : " << *numSteps << " Restroe path A*" << endl;
	return pc;
}
...
Position NPC::RunAStarFlee(int dupMaze[MSZ][MSZ])
{
	...
	{
		std::cout << "Error pq is empty, (RunAStarFlee)" << endl;
	}
	return nextPos;
}
...
Position NPC::FindFreeSpaceToMoveInLoop()
{
	...
	{
		std::cout << "\nNo free space to move in loop" << endl;
		return nextPos;
	}
}
...
Position NPC::getEntranceToCorridor(int corridorIndex, int roomIndex, bool forSecMap)
{
	...
	if (corridor == nullptr)
	{
		std::cout << "Error: corridor not found" << endl;
		return { -1, -1 };
	}
	...
	std::cout << "Error: entrance not found" << endl;
	return { -1, -1 };
}
